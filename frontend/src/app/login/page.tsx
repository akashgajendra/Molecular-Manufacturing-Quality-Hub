...
import { Microscope, LockKeyhole, ArrowRight, ShieldAlert, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginValues) => {
    ...
    try {
      ...
      setAuthError("Critical Uplink Failure: Node unreachable.");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <CardContent className="pt-8 px-8 pb-6">
            ...
            <AnimatePresence>
              {authError && (
                <motion.div 
                  ...
                  className="mb-6 bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl flex items-center gap-3"
                >
                  <ShieldAlert className="w-4 h-4 text-rose-500 shrink-0" />
                  <p className="text-[11px] font-bold text-rose-400 uppercase tracking-widest leading-tight">
                    {authError}
                  </p>
                </motion.div>
              )}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1 text-left block">System ID</label>
                <Input 
                  {...register("username")} 
                  disabled={isLoading}
                  className="bg-slate-950 border-slate-800 text-white h-14 px-4 text-base focus:ring-2 focus:ring-indigo-500/50 disabled:opacity-50" 
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  type="password" 
                  disabled={isLoading}
                  className="bg-slate-950 border-slate-800 text-white h-14 px-4 text-base disabled:opacity-50" 
                  placeholder="••••••••" 
                />
              ...
              <Button 
                type="submit" 
                disabled={isLoading}
                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white h-14 text-base font-bold uppercase tracking-widest shadow-xl shadow-indigo-600/20 group mt-2 transition-all"
              >
                {isLoading ? (
